...
# --- 1. การตั้งค่าหน้าเว็บ (ต้องอยู่บรรทัดแรกสุด) ---
st.set_page_config(
    page_title="PM 2.5 Monitor | BUU", 
    page_icon="🌊", 
    layout="wide", # แสดงผลแบบเต็มจอ
    initial_sidebar_state="collapsed"
)

# --- ซ่อนเมนูพื้นฐานของ Streamlit ด้วย CSS ---
hide_st_style = """
            <style>
...
def get_regional_pm25_forecast():
    """ดึงข้อมูลคาดการณ์ PM 2.5 พื้นที่ ม.บูรพา จาก Open-Meteo"""
    LAT = 13.28
    LON = 100.92
...
with tab1:
    if current_data:
        ...
        # กล่องแจ้งเตือนสถานะฝุ่น
        if pm_val <= 15.0:
            st.success("🟢 **คุณภาพอากาศดีมาก:** สามารถทำกิจกรรมกลางแจ้งได้ตามปกติ")
        elif pm_val <= 37.5:
            st.info("🟡 **คุณภาพอากาศปานกลาง:** ประชาชนทั่วไปทำกิจกรรมกลางแจ้งได้ กลุ่มเสี่ยงควรระวัง")
        elif pm_val <= 75.0:
            st.warning("🟠 **เริ่มมีผลกระทบต่อสุขภาพ:** ควรลดระยะเวลาการทำกิจกรรมกลางแจ้ง หรือสวมหน้ากากอนามัย")
        else:
            st.error("🔴 **มีผลกระทบต่อสุขภาพ:** งดกิจกรรมกลางแจ้ง และสวมหน้ากากอนามัย N95 ทันที!")

        st.markdown("### ข้อมูลสภาพอากาศล่าสุด")
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric(label="PM 2.5 (ปรับแก้แล้ว)", value=f"{pm_val} µg/m³", 
                      delta=f"{round(pm_val - current_data['raw_pm25'], 2)} (ชดเชยค่าความชื้น)")
        with col2:
            st.metric(label="PM 2.5 (ค่าดิบก่อนแก้)", value=f"{current_data['raw_pm25']} µg/m³")
        with col3:
...
with tab2:
    ...
    with st.spinner('กำลังโหลดข้อมูลพยากรณ์...'):
        ...
        if forecast_df is not None and not forecast_df.empty:
            ...
            # ตาราง (แก้ไขโค้ดเรื่องคอลัมน์แล้ว)
            display_df = forecast_df[['date', 'Regional_PM25']].copy()
            display_df.columns = ['วันที่คาดการณ์', 'ค่า PM 2.5 ประเมิน (µg/m³)']
            st.dataframe(display_df, use_container_width=True, hide_index=True)

with tab3:
    st.subheader("ข้อมูลสถาปัตยกรรมระบบ (System Architecture)")
    st.markdown("""
    * **ฮาร์ดแวร์:** ESP32 Board + PMS5003 (Dust Sensor) + BME280 (Temp/Humidity)
    * **การกรองสัญญาณดิบ:** อัลกอริทึม Trimmed Mean (ตัดขอบสัญญาณรบกวน 5 Sample)
    * **Cloud Database:** ThingSpeak Channel API
    """)
    
    st.info(f"**สมการทางคณิตศาสตร์ (Calibration):** $y = mx + c$  \n(กำหนดค่า $m = {SLOPE_M}$, $c = {INTERCEPT_C}$ | ระบบหักลบไอน้ำเมื่อความชื้นสูงเกิน {HUMIDITY_THRESHOLD}%)")
    
    st.markdown("---")
    st.caption("พัฒนาโดย: โครงงานศูนย์เฝ้าระวังคุณภาพอากาศ ม.บูรพา บางแสน | ทำงานร่วมกับระบบแจ้งเตือน LINE อัตโนมัติ 24 ชม.")
